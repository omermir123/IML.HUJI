...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        all_weights = [f.weights]
        for t in range(self.max_iter_):
            all_weights.append(all_weights[-1] - self.learning_rate_.lr_step(t=t) * f.compute_jacobian(X=X, y=y))
            f.weights = all_weights[-1]
            cur_weight_val = f.compute_output(X=X, y=y)
            cur_weight_jacobian = f.compute_jacobian(X=X, y=y)
            norm = np.linalg.norm(all_weights[-1] - all_weights[-2])
            self.callback_(GD=self, weight=all_weights[-1], value=cur_weight_val, jacobian=cur_weight_jacobian, t=t,
                           eta=self.learning_rate_.lr_step(t=t), delta=norm)
            if norm < self.tol_:
                break
